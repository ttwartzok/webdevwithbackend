...
if ($stmt = $con->prepare('SELECT id, userpassword FROM customers WHERE username = ?')) {
	...
	if ($stmt->num_rows > 0) {
        ...
        if ($stmt = $con->prepare('INSERT INTO customers (username, userpassword, email) VALUES (?, ?, ?)')) {	// We do not want to expose passwords in our database, so hash the password and use password_verify when a user logs in.
